 ...
 */
 var accountsMerge = function(accounts) {
    ...
    const map = {}
    const unionSet = (emails, ...sets) => {

        for (const merge of sets) {
            emails.push(...merge)
        }
    ...
    for(let i = 0; i < accounts.length; i++) {
       ...
       map[name] = map[name].filter(s => !merge.includes(s))
       map[name].push(unionSet(emails, ...merge))
    }
    ...
    for(const name in map) {
        ...
        for (const emailsSet of map[name]) {

            res.push([name, ...[...emailsSet].sort() ])
        }
