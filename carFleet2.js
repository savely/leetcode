...
Two cars collide when they occupy the same position. Once a car collides with another car, they unite and form a single car fleet. 
...
Return an array answer, where answer[i] is the time, in seconds, at which the ith car collides with the next car, or -1 if the car does not 
collide with the next car. Answers within 10-5 of the actual answers are accepted.
 ...
 */
 var getCollisionTimes = function(cars) {
   ...
   colTimes  = new Array(cars.length).fill(-1)
   ...
   for(let i = cars.length - 2; i >= 0; i--) {
    ...
    const  [pos, speed] = cars[i]
        ...
        for(let j = i + 1; j < cars.length; j++) {
           ...
           const [fwdPos, fwdSpeed] = cars[j]

           if(speed <= fwdSpeed && colTimes[j] < 0) break;
           
           if(speed <= fwdSpeed) continue;

           const fwdColTime = (fwdPos - pos) / (speed - fwdSpeed)            

           if(colTimes[j] < 0 || colTimes[j] >= fwdColTime) {
               colTimes[i] = fwdColTime
               break;
           }
        }
   }
...
let cars = [[1,2],[2,1],[4,3],[7,2]];
cars = [[3,4],[5,4],[6,3],[9,1]];
cars = [[3,1],[9,4],[19,4]];

console.table(getCollisionTimes(cars))