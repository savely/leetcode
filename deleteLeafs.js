...
var removeLeafNodes = function(root, target) {
    ...
    while(stack.length) {
        ...
        while(stack[stack.length - 1].left 
            && stack[stack.length - 1].left.val > -1){
        ...
        if(stack[stack.length - 1].right
            && stack[stack.length - 1].right.val > -1) {
        ...
        if(node.val === -1 * target && !node.left && !node.right) {

            if(stack.length && stack[stack.length - 1].left === node) {
