 ...
 */
 var findAllPeople = function(n, meetings, firstPerson) {
    ...
    const times = {0 : new Set([0])};

    for(const [person1, person2, time] of meetings) {

        times[time] = times[time] || new Set();

        times[time].add(person1);
        times[time].add(person2);

    }

    ;

    const informed = new Set([0, firstPerson]);

    for(const [person1, person2, time] of meetings) {
        
        if(informed.has(person1)) informed.add(person2);

        if(informed.has(person2)) informed.add(person1);
    }
...
console.table(findAllPeople(n, meetings, firstPerson));