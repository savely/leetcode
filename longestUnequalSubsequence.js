...
var getWordsInLongestSubsequence = function(words, groups) {
    ...
    const n = words.length, dp = new Array(n).fill(1), prev = new Array(n).fill(-1);

    let maxLength = 1, maxIndex = 0;

    for(let i = 0; i < n; i++) {
        for(let j = 0; j < i; j++) {
            if(groups[i] !== groups[j] && isDistanceOne(words[i], words[j])) {
                if(dp[j] + 1 > dp[i]) {
                    dp[i] = dp[j] + 1;
                    prev[i] = j;
                }
