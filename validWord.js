...
var isValid = function(word) {
    ...
    const isDigit = (c) => !Number.isNaN(+c);

    const isEnglishLetter = (c) => {
        const code = c.toLowerCase().charCodeAt(0), a = 'a'.charCodeAt(), z = 'z'.charCodeAt();

        return  code >= a && code <= z;
    };

    isVowel = (c) => vowels.has(c.toLowerCase());
    ...
    for(const c of word) {
        ...
        if(isEnglishLetter(c)) {
            hasVowel ||= isVowel(c);
            hasConsonant ||= !isVowel(c);
        } else if(!isDigit(c)) {
            return false;
        }
    }

    return fhasVowel && hasConsonant;
};
